...
DEPENDENCES="ca-certificates ecb emacs-apel emacs-muse emacs-php-mode emacs-python-mode emacs-slime flim semi wanderlust"
...
VERSION=$(wget -q https://archlinux.org/packages/extra/x86_64/$APP/ -O - | grep $APP | grep details | head -1 | grep -o -P '(?<=">).*(?=</)' | grep -o '^\S*')
...
LIBSAVED="pk p11 emacs" # Enter here keywords or file/folder names to save in /usr/lib
